...
static t_file_state *initialize_file_state(t_file_state **file_state, char **line)
{
	if(*file_state == NULL)
	{
...
static t_bool eof_or_error(int *bytes_read, char **line, t_file_state **file_state, int *ret)
{
	if(*bytes_read <= 0)
	{
		if(*bytes_read == 0)
