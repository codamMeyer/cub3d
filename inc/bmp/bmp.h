...
/* __attribute__((__packed__)) is to guarantee that no padding will be added to the struct, so the struct has only 14 bytes */
...
/* __attribute__((__packed__)) is to guarantee that no padding will be added to the struct, so the struct has only 40 bytes */
