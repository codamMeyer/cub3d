...
CTEST_DATA(player_backward_move)
{
	t_data game;
};

CTEST_SETUP(player_backward_move)
{
...
CTEST_TEARDOWN(player_backward_move)
{
	free_matrix(data->game.worldMap.matrix, data->game.worldMap.height);
}

CTEST2(player_backward_move, player_facing_90_degrees)
{
	int expected_x = 1;
	int expected_y = 3;
	...
	while (1)
	{
		if (!move_backward(&data->game.player, data->game.worldMap))
			break;
	}
...
CTEST2(player_backward_move, player_facing_0_degrees)
{
	int expected_x = 1;
	int expected_y = 3;

	data->game.player.position.x = 186;
	data->game.player.position.y = 120;
...
CTEST2(player_backward_move, player_facing_0_degrees_with_obstacle)
{
	int expected_x = 4;
	int expected_y = 2;
...
CTEST2(player_backward_move, player_facing_270_degrees)
{
	int expected_x = 1;
	int expected_y = 1;
...
CTEST2(player_backward_move, player_facing_230_degrees)
{
	int expected_x = 3;
	int expected_y = 1;

	data->game.player.position.x = 48;
	data->game.player.position.y = 100;
...
CTEST2(player_backward_move, player_facing_220_degrees)
{
	int expected_x = 5;
	int expected_y = 1;
...
CTEST2(player_backward_move, player_facing_180_degrees)
{
	int expected_x = 5;
	int expected_y = 1;
